...
public class Assembler {
	...
	public static int assemble(File input, File output, StringBuilder error) {
		...
		try (Scanner inp = new Scanner(input)) {
			...
			// a while loop to read input, stopping two of the simplest errors found
			//current line number
			int currentLine = 0;
			//line number of the first blank line found
			int blankLine = 0;
			//if a blank line has been found
			int blanksFound = 0;
			/////////////////////////
			...
			while(inp.hasNextLine() && retVal == 0) {
				...
				if(line.trim().length() == 0){
					blankLine = currentLine;
					blankLineFound = true;
					blanksFound += 1;
				}else{
					if(blankLineFound == true && blankLine != currentLine){
						error.append("Illegal blank line in the source file");
						retVal = currentLine - blanksFound;
					}else{
						...
						if(parts[0] != parts[0].toUpperCase()){
							...
							if(parts.length>2){
								...
								try{
									...
									error.append("Error on line " + (i+1) + ": argument is not a hex number1");
									retVal = i+1;
								}
							}
						}
			...
			for(int i = 0; i < inputData.size() && retVal == 0; i++){
				if(retVal == 0){
					String[] dataParts = inputData.get(i).split("\\s+");
					if(dataParts.length != 2){
						error.append("Error on line " + (i+1) + ": There are not 2 elements in the data line" );
						retVal = i+1;
					}else{
						try{
							int arg1 = Integer.parseInt(dataParts[0],16);
							int arg2 = Integer.parseInt(dataParts[1],16);
							outputData.add(Integer.toString(arg1, 16) + " " + Integer.toString(arg2, 16));
						}catch(NumberFormatException n){
							error.append("Error on line " + (i+1) + ": one of the arguments is not a hex number");
							retVal = i+1;
						}
					}
				}
